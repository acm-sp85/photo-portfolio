...
export function mapImageResources(resources) {
  const sortedResources = resources.sort((a, b) => {
    const nameA = a.public_id.toLowerCase();
    const nameB = b.public_id.toLowerCase();
  ...
  return sortedResources.map((resource) => {
    const { width, height, filename, folder } = resource;
...
export async function getFolders(options = {}) {
  ...
  const results = await fetch(
    `https://api.cloudinary.com/v1_1/${process.env.CLOUDINARY_CLOUD_NAME}/folders/${options}`,
    {
      headers: {
        Authorization: `Basic ${Buffer.from(
          process.env.CLOUDINARY_API_KEY +
